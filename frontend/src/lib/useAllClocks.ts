...
//
// State-only (App.tsx pattern). The two handlers — handleOpenAllClocks
// (parallel-fetch clock + firmware across all switches) and
// handleSubmitNtpSync (create + open a plan that sets NTP on every
// switch with a parseable response) — stay in App.tsx because they
// close over App-scope helpers (closeAllTier3Widgets, switchOptions,
// setPlanDetail, setPlanDetailOpen) that aren't part of
// this hook's surface.
//
// Pattern matches lib/useFwUpg — see the comment in CLAUDE.md about
// when to extract state-only hooks vs full feature hooks.
//
